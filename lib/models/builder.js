...
var BUILD_INSTRUMENTATION = require('../features/build-instrumentation');
...
function instrumentationEnabled() {
  return !!process.env.EMBER_CLI_INSTRUMENTATION;
}

function computeVizInfo(count, result, resultAnnotation) {
  var tree = require('heimdalljs-tree');
  var buildTree = tree.loadFromNode(result.graph.__heimdall__);
  var buildSteps = Array.from(buildTree.preOrderIterator()).filter(function(x) {
    return x.label.broccoliNode;
  }).length;
  ...
  build: function(willReadStringDir, resultAnnotation) {
    ...
    return this.processAddonBuildSteps('preBuild')
      ...
      .then(function(result) {
        ...
        if (vizEnabled() || instrumentationEnabled()) {
          vizJson = computeVizInfo(buildCount, result, resultAnnotation);
          self._reportVizInfo(vizJson);
        }

        if (vizEnabled()) {
          writeVizInfo(buildCount, vizJson);
        }
