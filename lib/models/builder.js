...
function computeVizInfo(count, result, resultAnnotation) {
  ...
  var buildSteps = [...buildTree.preOrderIterator()].filter(x => x.label.broccoliNode).length;
...
module.exports = CoreObject.extend({
  ...
  */
  ...
  copyToOutputPath: function(inputPath) {
    ...
    return changes.map(function (op) { return op[1]; } );
  },
  ...
  _reportVizInfo: function (vizInfo) {
    if (this.project && this.project.addons.length) {
      this.project.addons.forEach(function (addon) {
        if (addon._buildAnalytics) {
          addon._buildAnalytics(vizInfo);
        }
      });
    }
  },
  ...
  build: function(willReadStringDir, resultAnnotation) {
    ...
    return this.processAddonBuildSteps('preBuild')
      ...
      .then(function(result) {
        if (vizEnabled()) {
          var vizJson = computeVizInfo(buildCount, result, resultAnnotation);
